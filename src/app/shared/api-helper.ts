...
export class ApiHelper {
   ...
   */
  static getConfirmationMessage(passwordInput: PasswordInput, i18n: I18n) {
    ...
    if (otp) {
      if (!hasOtpSendMediums) {
        return i18n(`In order to confirm you need a {{name}}, but you cannot request a new password.
          Please, contact the administration.`, {
            name: name
          });
      } else if (activePassword) {
        return i18n(`In order to confirm, please, supply your {{name}}.
          You can use the previously sent password or request for a new one.`, {
            name: name
          });
      } else {
        return i18n('Please, request a password below in order to confirm.');
      }
    } else {
      return i18n(`In order to confirm you need a {{name}}, but you do not have any.`, {
        name: name
      });
    }
  }
