...
export class RecordFormComponent extends BasePageComponent<RecordDataForEdit | RecordDataForNew> implements OnInit {
  ...
  constructor(injector: Injector, public recordsService: RecordsService, public recordsHelper: RecordHelperService) {
    super(injector);
  }
  ...
  onDataInitialized(data: RecordDataForEdit | RecordDataForNew) {
    ...
    this.form.setControl(
      'customValues',
      this.fieldHelper.customValuesFormGroup(data.fields, {
        currentValues: data.record.customValues,
        disabledProvider: cf =>
          !this.create && ((data as RecordDataForEdit).editableFields || []).indexOf(cf.internalName) === -1
      })
    );
  }
    ...
    this.addSub(
      request.subscribe(id => {
        this.notification.snackBar(
          this.create ? this.i18n.record.created(this.data.type.name) : this.i18n.record.saved(this.data.type.name)
        );
