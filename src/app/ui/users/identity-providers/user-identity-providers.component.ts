...
export class UserIdentityProvidersComponent extends BasePageComponent<UserIdentityProvidersListData> implements OnInit {
  param: string;
  self: boolean;
  ...
  constructor(injector: Injector, private identityProvidersService: IdentityProvidersService) {
    super(injector);
  }
  ...
  ngOnInit() {
    ...
    this.addSub(
      this.identityProvidersService.getUserIdentityProvidersListData({ user: this.param }).subscribe(data => {
        this.data = data;
      })
    );
  }
  ...
  onDataInitialized(data: UserIdentityProvidersListData) {
    ...
    this.title = this.self ? this.i18n.identityProvider.title.manageSelf : this.i18n.identityProvider.title.manageUser;
  ...
  actions(uip: UserIdentityProvider): Action[] {
    ...
    if (uip.status !== UserIdentityProviderStatusEnum.LINKED && this.data.canLink) {
      actions.push(
        new Action(this.i18n.identityProvider.action.connect, () => {
          this.connect(uip);
        })
      );
    }
    if (uip.status === UserIdentityProviderStatusEnum.LINKED && this.data.canEdit) {
      actions.push(
        new Action(this.i18n.identityProvider.action.disconnect, () => {
          this.disconnect(uip);
        })
      );
    }
  ...
  private connect(uip: UserIdentityProvider) {
    this.addSub(
      this.authHelper.identityProviderPopup(uip.identityProvider, 'link').subscribe(callback => {
        this.notification.snackBar(
          this.i18n.identityProvider.action.connectDone({
            name: callback.name,
  ...
  resolveMenu(data: UserIdentityProvidersListData) {
    return this.menu.userMenu(data.user, Menu.IDENTITY_PROVIDERS);
  }
}
