  ...
  implements OnInit
{
  ...
  ngOnInit() {
    ...
    const newAction = this.canSend
      ? new HeadingAction(
          this.SvgIcon.Envelope,
          this.i18n.message.actions.newMessage,
            ...
            headingActions.push(
              new HeadingAction(
                ...
                this.form.controls.messageBox.value === MessageBoxEnum.TRASH
                  ...
                  : this.i18n.message.actions.moveAllToTrash,
                () => this.removeAll()
              )
            );
          }
  ...
  get canSend(): boolean {
    ...
    return my.sendToUser || my.sendToSystem || my.sendToBrokered || system.sendToGroups || system.sendToUser;
  }
  ...
  remove(toTrash: boolean, message: Message) {
    if (!toTrash) {
      this.confirmation.confirm({
        ...
        callback: () =>
          this.addSub(
            this.messagesService.deleteMessage({ id: message.id }).subscribe(() => {
      ...
      this.addSub(
        this.messagesService.moveMessagesToTrash({ ids: [message.id] }).subscribe(() => {
