...
import { CustomFieldTypeEnum, PaymentFeedbackView } from 'app/api/models';
...
export class ViewFeedbackComponent extends BaseViewPageComponent<PaymentFeedbackView> implements OnInit {
  ...
  ngOnInit() {
    ...
    this.addSub(this.feedbackService.viewPaymentFeedback({ key: this.id }).subscribe(data => (this.data = data)));
  }
  ...
  onDataInitialized(data: PaymentFeedbackView) {
    ...
    if (data.canEdit || data.canGive) {
      actions.push(
        new HeadingAction(
          ...
          () => this.router.navigate(['/users', 'feedbacks', 'edit', this.id]),
          true
        )
      );
      if (data.canEdit) {
      ...
      if (data.canGive) {
        this.addSub(
          this.feedbackService
            .getPaymentFeedbacksDataForGive({ key: this.id, fields: ['deadline'] })
            .subscribe(giveData => {
    ...
    if (data.canReply) {
      actions.push(new HeadingAction(SvgIcon.Reply, this.i18n.feedback.reply, () => this.reply(data.id)));
      this.addSub(
        this.feedbackService
          .getPaymentFeedbacksDataForReply({ key: this.id, fields: ['deadline'] })
          .subscribe(replyData => {
  ...
  protected doRemove(id: string) {
    this.addSub(
      this.feedbackService.removePaymentFeedback({ key: id }).subscribe(() => {
