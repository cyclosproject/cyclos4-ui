...
export class AdFavoriteComponent
  ...
  setFavorite(ad: AdResult): void {
    if (ad.favoriteForViewer) {
      this.addSub(this.marketplaceService.unmarkAsFavorite({ ad: ad.id })
        .subscribe(() => {
          this.notification.snackBar(this.i18n.ad.removedFromFavorites);
        }));
    } else {
      this.addSub(this.marketplaceService.markAsFavorite({ ad: ad.id })
        .subscribe(() => {
