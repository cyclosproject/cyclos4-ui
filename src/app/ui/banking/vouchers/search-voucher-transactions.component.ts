...
export class SearchVoucherTransactionsComponent
  ...
  onDataInitialized(data: UserVoucherTransactionsDataForSearch) {
    ...
    if (data.canRedeem) {
      headingActions.push(new HeadingAction(SvgIcon.TicketRedeem, this.i18n.voucher.redeem.redeem, () => this.redeem(), true));
    }
    if (data.canTopUp) {
      headingActions.push(new HeadingAction(SvgIcon.TicketTopUp, this.i18n.voucher.topUp.topUp, () => this.topUp(), true));
    }
