...
export class ViewVoucherComponent extends BaseViewPageComponent<VoucherView> implements OnInit {
  ...
  onDataInitialized(data: VoucherView) {
    if (data.showQrCode) {
      this.addSub(
        this.voucherService
          .getVoucherQrCode({ key: data.id, size: ImageSizeEnum.MEDIUM })
  ...
  updateTransactions() {
    this.addSub(
      this.voucherService
        .searchVoucherTransactions$Response({ key: this.data.id })
        .subscribe(r => this.transactions$.next(PagedResults.from(r)))
    );
  }

  initActions(voucher: VoucherView): HeadingAction[] {
    const actions: HeadingAction[] = this.exportHelper.headingActions(voucher.exportFormats, f =>
      this.voucherService.exportVoucher$Response({
    ...
    if (voucher.cancelAction) {
      const label =
        voucher.cancelAction === VoucherCancelActionEnum.CANCEL_AND_REFUND
      ...
      actions.push(
        new HeadingAction(SvgIcon.XCircle, label, () => {
    ...
    if (voucher.canAssign) {
      actions.push(
        new HeadingAction(SvgIcon.PersonBadge, this.i18n.voucher.assign, () => {
          // A confirmation is required
          this.confirmation.confirm({
    ...
    if (voucher.canChangeExpirationDate) {
      actions.push(
        new HeadingAction(SvgIcon.CalendarEvent, this.i18n.voucher.changeExpirationDate, () => {
          // Handle the case that the confirmation password cannot be used
          if (!this.canConfirm) {
            this.notification.warning(this.authHelper.getConfirmationMessage(voucher.confirmationPasswordInput));
            return;
    ...
    if (voucher.canResendEmail) {
      actions.push(
        new HeadingAction(SvgIcon.At, this.i18n.voucher.resendEmail.label, () => {
          // Handle the case that the confirmation password cannot be used
          if (!this.canConfirm) {
            this.notification.warning(this.authHelper.getConfirmationMessage(voucher.confirmationPasswordInput));
            return;
    ...
    if (voucher.canChangeNotificationSettings) {
      actions.push(
        new HeadingAction(SvgIcon.Gear, this.i18n.voucher.notificationSettings.label, () => {
          // Handle the case that the confirmation password cannot be used
    ...
    if (voucher.canChangePin) {
      actions.push(
        new HeadingAction(SvgIcon.Key, this.i18n.voucher.changePin.label, () => {
          if (!voucher.requireOldPinForChange && !this.canConfirm) {
            this.notification.warning(this.authHelper.getConfirmationMessage(voucher.confirmationPasswordInput));
            return;
          }
    ...
    if (voucher.canUnblockPin) {
      actions.push(
        new HeadingAction(SvgIcon.Unlock, this.i18n.voucher.unblockPin.unblockPin, () => {
          this.addSub(
            this.voucherService
              .unblockVoucherPin({
                key: voucher.id
  ...
  cancel(confirmationPassword?: string) {
    ...
    this.addSub(
      this.voucherService.cancelVoucher({ key: this.data.id, confirmationPassword }).subscribe(() => {
        this.reload();
  ...
  transactionPath(transfer: Transaction): string[] {
    if (
      transfer.authorizationStatus === null ||
  ...
  resolveMenu(voucher: VoucherView) {
    ...
    if (isBuyer || isOwner) {
      return this.dataForFrontendHolder.dataForFrontend.voucherBuyingMenu == UserMenuEnum.MARKETPLACE
        ? Menu.SEARCH_MY_VOUCHERS_MARKETPLACE
        : Menu.SEARCH_MY_VOUCHERS_BANKING;
    } else if (
      auth?.permissions?.vouchers?.vouchers?.find(
        p => p.configuration.id === voucher?.type?.configuration?.id && p.view
      )
    ) {
