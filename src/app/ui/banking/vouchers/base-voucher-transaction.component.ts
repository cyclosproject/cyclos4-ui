...
export type VoucherTransactionStep = 'token' | 'form' | 'confirm' | 'done';
...
export abstract class BaseVoucherTransactionComponent<
  ...
  // Step 4: done
  result$ = new BehaviorSubject<VoucherTransactionResult>(null);
  get result(): VoucherTransactionResult {
  ...
  printResult() {
    ...
    this.addSub(this.vouchersService.exportVoucherTransaction$Response({
      format: 'pdf', id
    }).subscribe(r => downloadResponse(r)));
  }
