...
import {
  AddressNew, IdentityProvider, IdentityProviderCallbackStatusEnum,
  PhoneNew, UserNew, WizardActionEnum, WizardExecutionData, WizardKind,
  WizardResultTypeEnum, WizardStepKind, WizardStepTransition, AvailabilityEnum
} from 'app/api/models';
  ...
  resolveMenu(data: WizardExecutionData) {
    ...
    switch (kind) {
      ...
      case WizardKind.SYSTEM:
        return new ActiveMenu(ApiHelper.menuForWizard(wizard), { wizard });
      case WizardKind.USER:
        return this.authHelper.userMenu(data.user,
          new ActiveMenu(ApiHelper.menuForWizard(wizard), { wizard }));
    }
  }
